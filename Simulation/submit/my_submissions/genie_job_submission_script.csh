...
echo "${PRINT_OUT_COLOR}SAMPLE_TARGET_NUCLEUS:\033[0m ${SAMPLE_TARGET_NUCLEUS}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

echo "${PRINT_OUT_COLOR}GENIE_TUNE:\033[0m ${GENIE_TUNE}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

echo "${PRINT_OUT_COLOR}Q2_CUT:\033[0m ${Q2_CUT}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

echo "${PRINT_OUT_COLOR}BEAM_E:\033[0m ${BEAM_E}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

echo "${PRINT_OUT_COLOR}FC_STATUS:\033[0m ${FC_STATUS}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

echo "${PRINT_OUT_COLOR}FC_STATUS_ENABLED:\033[0m ${FC_STATUS_ENABLED}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

echo "${PRINT_OUT_COLOR}OUTPATH:\033[0m ${OUTPATH}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

# Check if OUTPATH is a directory
if (! -d "${OUTPATH}") then
    echo "Error: Directory specified by OUTPATH does not exist: ${OUTPATH}" >> ${LOG_FILE}
    exit 1
endif

echo "${PRINT_OUT_COLOR}- Job parameters ------------------------------------------------------\033[0m" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

echo "${PRINT_OUT_COLOR}TORUS_FIELD:\033[0m ${TORUS_FIELD}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

unsetenv CLEAR_FARM_OUT
setenv CLEAR_FARM_OUT 0 ## 1 for true
echo "${PRINT_OUT_COLOR}CLEAR_FARM_OUT:\033[0m ${CLEAR_FARM_OUT}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

unsetenv CANCEL_PREVIOUS_JOBS
setenv CANCEL_PREVIOUS_JOBS 0 ## 1 for true
echo "${PRINT_OUT_COLOR}CANCEL_PREVIOUS_JOBS:\033[0m ${CANCEL_PREVIOUS_JOBS}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

unsetenv USE_GEMC_5_10
setenv USE_GEMC_5_10 1 ## 1 for true
echo "${PRINT_OUT_COLOR}USE_GEMC_5_10:\033[0m ${USE_GEMC_5_10}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

# Setting RUNNING_DIR
unsetenv RUNNING_DIR
setenv RUNNING_DIR `pwd`
echo "${PRINT_OUT_COLOR}RUNNING_DIR::\033[0m ${RUNNING_DIR}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

# Check if RUNNING_DIR is a directory
if (! -d "${RUNNING_DIR}") then
    echo "Error: Directory specified by RUNNING_DIR does not exist: ${RUNNING_DIR}" >> ${LOG_FILE}
    exit 1
endif
...
if ("${BEAM_E}" == "2070MeV") then
    echo "${PRINT_OUT_COLOR}- Setting SUBMIT_SCRIPT_PATH for 2 GeV --------------------------------\033[0m" >> ${LOG_FILE}
    setenv SUBMIT_SCRIPT_PATH ${RUNNING_DIR}/Uniform_sample_2GeV
    echo "" >> ${LOG_FILE}
else if ("${BEAM_E}" == "4029MeV") then
    echo "${PRINT_OUT_COLOR}- Setting SUBMIT_SCRIPT_PATH for 4 GeV --------------------------------\033[0m" >> ${LOG_FILE}
    setenv SUBMIT_SCRIPT_PATH ${RUNNING_DIR}/Uniform_sample_4GeV
    echo "" >> ${LOG_FILE}
else if ("${BEAM_E}" == "5986MeV") then
    echo "${PRINT_OUT_COLOR}- Setting SUBMIT_SCRIPT_PATH for 6 GeV --------------------------------\033[0m" >> ${LOG_FILE}
    setenv SUBMIT_SCRIPT_PATH ${RUNNING_DIR}/Uniform_sample_6GeV
    echo "" >> ${LOG_FILE}
endif

echo "${PRINT_OUT_COLOR}SUBMIT_SCRIPT_PATH:\033[0m ${SUBMIT_SCRIPT_PATH}" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}

# Check if SUBMIT_SCRIPT_PATH is a directory
if (! -d "${SUBMIT_SCRIPT_PATH}") then
    echo "Error: Directory specified by SUBMIT_SCRIPT_PATH does not exist: ${SUBMIT_SCRIPT_PATH}" >> ${LOG_FILE}
    exit 1
endif
...
echo "${PRINT_OUT_COLOR}GCARD_FILE:\033[0m ${GCARD_FILE}" >> ${LOG_FILE}
echo

# Check if GCARD_FILE is a file
if (! -f "${GCARD_FILE}") then
    echo "Error: File specified by GCARD_FILE does not exist: ${GCARD_FILE}" >> ${LOG_FILE}
    exit 1
endif
...
echo "${PRINT_OUT_COLOR}YAML_FILE:\033[0m ${YAML_FILE}" >> ${LOG_FILE}
echo

# Check if YAML_FILE is a file
if (! -f "${YAML_FILE}") then
    echo "Error: File specified by YAML_FILE does not exist: ${YAML_FILE}" >> ${LOG_FILE}
    exit 1
endif

# Re-pulling repository
echo "${PRINT_OUT_COLOR}- Re-pulling repository -----------------------------------------------\033[0m" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}
echo "${PRINT_OUT_COLOR}Pulling updates...\033[0m" >> ${LOG_FILE}
...
if ("${CLEAR_FARM_OUT}" == "1") then
    echo "${PRINT_OUT_COLOR}- Clearing farm_out directory -----------------------------------------\033[0m" >> ${LOG_FILE}
...
if ("${CANCEL_PREVIOUS_JOBS}" == "1") then
    echo "${PRINT_OUT_COLOR}- Canceling previous jobs ---------------------------------------------\033[0m" >> ${LOG_FILE}
...
if ("${USE_GEMC_5_10}" == "1") then
    echo "${PRINT_OUT_COLOR}- Reverting to GEMC 5.10 ----------------------------------------------\033[0m" >> ${LOG_FILE}
    module unload gemc
    module load gemc/5.10
    echo
endif

echo "${PRINT_OUT_COLOR}GEMC_DATA_DIR:\033[0m ${GEMC_DATA_DIR}" >> ${LOG_FILE}
echo

# Check if GEMC_DATA_DIR is a directory
if (! -d "${GEMC_DATA_DIR}") then
    echo "Error: File specified by GEMC_DATA_DIR does not exist: ${GEMC_DATA_DIR}" >> ${LOG_FILE}
    exit 1
endif

# Remove old output directories
echo "${PRINT_OUT_COLOR}- Removing old directory structure for MC simulation -------------------\033[0m" >> ${LOG_FILE}
rm -rf ${OUTPATH}/mchipo ${OUTPATH}/reconhipo
echo

# Create new output directories
echo "${PRINT_OUT_COLOR}- Setting up directory structure for MC simulation ---------------------\033[0m" >> ${LOG_FILE}
mkdir -p ${OUTPATH}/mchipo ${OUTPATH}/reconhipo
echo

if (! -d "${OUTPATH}/lundfiles" || "`ls -A ${OUTPATH}/lundfiles`" == "") then
    echo "Error: ${OUTPATH}/lundfiles must exist and must NOT be empty." >> ${LOG_FILE}
    exit 1
endif

# Check if output folders are valid
if (! -d "${OUTPATH}/mchipo" || "`ls -A ${OUTPATH}/mchipo`" != "") then
    echo "Error: ${OUTPATH}/mchipo must exist and be empty before proceeding." >> ${LOG_FILE}
    exit 1
endif
 
# Check if output folders are valid
if (! -d "${OUTPATH}/reconhipo" || "`ls -A ${OUTPATH}/reconhipo`" != "") then
    echo "Error: ${OUTPATH}/reconhipo must exist and be empty before proceeding." >> ${LOG_FILE}
    exit 1
endif
 
# Check if the directories were created successfully
echo "${PRINT_OUT_COLOR}Number of files in target directory (OUTPATH):\033[0m" >> ${LOG_FILE}
echo "${PRINT_OUT_COLOR}Number of lund files:     \t\t\033[0m `ls ${OUTPATH}/lundfiles | wc -l`" >> ${LOG_FILE}
echo "${PRINT_OUT_COLOR}Number of mchipo files:   \t\t\033[0m `ls ${OUTPATH}/mchipo | wc -l`" >> ${LOG_FILE}
echo "${PRINT_OUT_COLOR}Number of reconhipo files:\t\t\033[0m `ls ${OUTPATH}/reconhipo | wc -l`" >> ${LOG_FILE}
echo

# Submitting job
echo "${PRINT_OUT_COLOR}- Submitting jobs ------------------------------------------------------\033[0m" >> ${LOG_FILE}
echo "" >> ${LOG_FILE}
echo "${PRINT_OUT_COLOR}Submitting GENIE sbatch job...\033[0m" >> ${LOG_FILE}

unsetenv SLURM_JOB_NAME
setenv SLURM_JOB_NAME ${SAMPLE_TARGET_NUCLEUS}_${GENIE_TUNE}_${BEAM_E}_${Q2_CUT}${FC_STATUS}
echo "${PRINT_OUT_COLOR}SLURM_JOB_NAME:\033[0m ${SLURM_JOB_NAME}" >> ${LOG_FILE}
